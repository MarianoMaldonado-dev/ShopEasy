...
export default function ProductDetailPage(){
  ...
  const { comments, questions, addComment, addQuestion, answerQuestion, deleteComment, deleteQuestion, addToCart, cartProducts } = useContext(DataContext)
  ...
  const onAddComment = () => {
    if (!currentUser) {
      setErrorMsg('Para realizar un comentario primero inicia sesión');
  ...
  const onAddQuestion = () => {
    if (!currentUser) {
      setErrorMsg('Inicia sesión en tu cuenta para realizar una pregunta');
      setTimeout(() => setErrorMsg(''), 2000);
      return;
    }
        ...
        <div style={{display:'flex',gap:'2rem',flexWrap:'wrap'}}>
          ...
          <div style={{flex:1}}>
            ...
            <button className="button" onClick={handleAddToCart}>
              {qtyInCart ? `Agregar otro (${qtyInCart})` : 'Añadir al carrito'}
            </button>
            {addedMsg && <div className="toast">{addedMsg}</div>}
